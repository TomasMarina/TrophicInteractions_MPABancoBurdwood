...
knitr::kable(round(prop_topol,2) %>% dplyr::select(Size, Links, LD, Connectance, Omnivory, PathLength), col.names = c("Species","Interactions","Density","Connectance","Omnivory","Path length"),
                      caption = "Global-level properties of the MPA Namuncurá-Banco Burdwood food web. See table 1 for definitions and ecological relevance.")
...
```{r figure4, echo=FALSE, fig.width = 20, fig.height=20, fig.cap="Species-level properties (A) betweenness, (B) closeness, (C) trophic similarity, and (D) topological role by trophic level. Linear regressions for betweenness ($y = 72.48x - 111.98, R^2 = 0.04, p-value < 3.38e-05$), closeness ($y = 5.78e-06x - 9.37e-04, R^2 = -0.0005, p-value = 0.37$) and trophic similarity ($y = -0.01x - 0.11, R^2 = 0.07, p-value = 6.76e-08$). Note that for A, B and C panels only species with TLs equal or greater than 2 were considered."}
...
plot_sp_tr <- data_TRColor %>% 
  ...
  geom_point(size=3, aes(color=TRColor)) +
